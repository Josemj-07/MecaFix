...
public class Product {
    ...
    public void setPrice(Price newPrice) {
        if (newPrice == null)
            throw new InvalidProductException("newPrice must not be null");
        this.price = newPrice;
    }

    public void increaseStock(int n) {
        if (n <= 0)
            throw new InvalidProductException("value must not be less than zero");
        this.stock += n;
    }
    ...
    public void decreaseStock(int n) {
        if (n <= 0)
            throw new InvalidProductException("value must not be less than zero");
        if (n > this.stock)
            throw new InvalidProductException("value must not be greater than the actual stock");
        this.stock -= n;
    }
